...
export default function RouteGuard({
  ...
  const isPublicRoute = () => {
    // Kiểm tra chính xác route
    if (publicRoutes.includes(pathname)) return true;

    // Kiểm tra route động (ví dụ: /products/123)
  ...
  useEffect(() => {
    ...
    if (!isLoading) {
      ...
      // Nếu route là /account nhưng người dùng không phải khách hàng
      if (pathname === "/account" && (!user || user.role !== "customer")) {
        router.push("/");
      }
    }
